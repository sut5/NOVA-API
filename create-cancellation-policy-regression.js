...
const collection = {
  info: {
    name: "Novas Cancellation Policy Regression Suite",
    schema: "https://schema.getpostman.com/json/collection/v2.1.0/collection.json"
  },
  variable: [
    { key: "invalid_uuid", value: "00000000-0000-0000-0000-000000000000" },
    { key: "malformed_uuid", value: "invalid-cancellation-policy-id" }
  ],
  item: [
    {
      name: "CANCELLATION POLICY | Login | Setup token",
      request: {
        method: "POST",
    ...
    {
      name: "CANCELLATION POLICY | GET Policies | Valid token",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/cancellation-policy?page=1&limit=5"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
        "var json = pm.response.json();",
        ...
        "  pm.test('Policies response has data', function(){ pm.expect(json.data).to.exist; });",
        "  var text = JSON.stringify(json);",
        "  var match = text.match(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i);",
        "  if(match){ pm.environment.set('cancellationPolicyId', match[0]); console.log('Cancellation Policy ID saved:', match[0]); }",
        "}",
        "pm.test('Response time acceptable', function(){ pm.expect(pm.response.responseTime).to.be.below(4000); });"
      ]}}]
    },
    {
      name: "CANCELLATION POLICY | GET Policies | Missing token",
      request: {
        method: "GET",
        header: [],
        url: "{{base_url}}/api/v1/cancellation-policy?page=1&limit=5"
    ...
    {
      name: "CANCELLATION POLICY | GET Policies | Invalid token",
      request: {
        method: "GET",
    ...
    {
      name: "CANCELLATION POLICY | GET Single Policy | Existing or saved id",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/cancellation-policy/{{cancellationPolicyId}}"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
        "var json = pm.response.json();",
    ...
    {
      name: "CANCELLATION POLICY | GET Single Policy | Invalid UUID",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/cancellation-policy/{{invalid_uuid}}"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
    ...
    {
      name: "CANCELLATION POLICY | GET Single Policy | Malformed UUID",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/cancellation-policy/{{malformed_uuid}}"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
    ...
    {
      name: "CANCELLATION POLICY | GET Policies | Invalid pagination",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/cancellation-policy?page=-1&limit=-5"
    ...
    {
      name: "CANCELLATION POLICY | GET Policies | SQL injection search",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/cancellation-policy?search=' OR '1'='1"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
    ...
    {
      name: "CANCELLATION POLICY | GET Policies | XSS search",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/cancellation-policy?search=<script>alert(1)</script>"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
    ...
    {
      name: "CANCELLATION POLICY | POST Create Policy | Empty payload validation",
      request: {
        method: "POST",
        header: [
        ...
        url: "{{base_url}}/api/v1/cancellation-policy",
        body: { mode: "raw", raw: "{}" }
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
        "var json = pm.response.json();",
...
fs.writeFileSync(
  "postman/Novas_CancellationPolicy_Regression.postman_collection.json",
  JSON.stringify(collection, null, 2)
);

console.log("Cancellation Policy regression collection created successfully");
