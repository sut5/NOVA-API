...
const collection = {
  info: {
    name: "Novas Booking Regression Suite",
    schema: "https://schema.getpostman.com/json/collection/v2.1.0/collection.json"
  },
  variable: [
    { key: "invalid_uuid", value: "00000000-0000-0000-0000-000000000000" },
    { key: "malformed_uuid", value: "invalid-booking-id" }
  ],
  item: [
    {
      name: "BOOKING | Login | Setup token",
      request: {
        method: "POST",
    ...
    {
      name: "BOOKING | GET Bookings | Valid token",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/bookings?page=1&limit=5"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
        "var json = pm.response.json();",
        ...
        "  pm.test('Bookings response has data', function(){ pm.expect(json.data).to.exist; });",
        "  var text = JSON.stringify(json);",
        "  var match = text.match(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i);",
        "  if(match){ pm.environment.set('bookingId', match[0]); console.log('Booking ID saved:', match[0]); }",
        "}",
        "pm.test('Response time acceptable', function(){ pm.expect(pm.response.responseTime).to.be.below(4000); });"
      ]}}]
    },
    {
      name: "BOOKING | GET Bookings | Missing token",
      request: {
        method: "GET",
        header: [],
        url: "{{base_url}}/api/v1/bookings?page=1&limit=5"
    ...
    {
      name: "BOOKING | GET Bookings | Invalid token",
      request: {
        method: "GET",
    ...
    {
      name: "BOOKING | GET Single Booking | Existing or saved id",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
    ...
    {
      name: "BOOKING | GET Single Booking | Invalid UUID",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/bookings/{{invalid_uuid}}"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
    ...
    {
      name: "BOOKING | GET Single Booking | Malformed UUID",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/bookings/{{malformed_uuid}}"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
    ...
    {
      name: "BOOKING | GET Calendar | Valid month/year",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/bookings/view/calendar?month=5&year=2026"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
        "pm.test('Calendar status acceptable', function(){ pm.expect([200,403,405]).to.include(pm.response.code); });",
        "pm.test('No server error', function(){ pm.expect(pm.response.code).to.be.below(500); });"
      ]}}]
    },
    {
      name: "BOOKING | GET Calendar | Invalid month",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/bookings/view/calendar?month=99&year=2026"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
        "pm.test('Invalid month handled safely', function(){ pm.expect([200,400,403,405,422]).to.include(pm.response.code); });",
        "pm.test('No server error', function(){ pm.expect(pm.response.code).to.be.below(500); });"
      ]}}]
    },
    {
      name: "BOOKING | GET Bookings | SQL injection filter",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/bookings?search=' OR '1'='1"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
    ...
    {
      name: "BOOKING | GET Bookings | XSS filter",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/bookings?search=<script>alert(1)</script>"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
    ...
    {
      name: "BOOKING | POST Create Booking | Empty payload validation",
      request: {
        method: "POST",
        header: [
...
fs.writeFileSync(
  "postman/Novas_Booking_Regression.postman_collection.json",
  JSON.stringify(collection, null, 2)
);

console.log("Booking regression collection created successfully");
