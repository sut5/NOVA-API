...
const collection = {
  info: {
    name: "Novas Partner Property Regression Suite",
    schema: "https://schema.getpostman.com/json/collection/v2.1.0/collection.json"
  },
  variable: [
    { key: "invalid_uuid", value: "00000000-0000-0000-0000-000000000000" },
    { key: "malformed_uuid", value: "invalid-partner-id" }
  ],
  item: [
    {
      name: "PARTNER PROPERTY | Login | Setup token",
      request: {
        method: "POST",
    ...
    {
      name: "PARTNER PROPERTY | GET Partners | Valid token",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/admin/partners?page=1&limit=5"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
        "var json = pm.response.json();",
        ...
        "if(pm.response.code === 200){ pm.test('Partners response has data', function(){ pm.expect(json.data).to.exist; }); }",
        "pm.test('Response time acceptable', function(){ pm.expect(pm.response.responseTime).to.be.below(4000); });"
      ]}}]
    },
    {
      name: "PARTNER PROPERTY | GET Branches Properties | Valid token",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
    ...
    {
      name: "PARTNER PROPERTY | GET Partners | Missing token",
      request: {
        method: "GET",
        header: [],
        url: "{{base_url}}/api/v1/admin/partners?page=1&limit=5"
    ...
    {
      name: "PARTNER PROPERTY | GET Partners | Invalid token",
      request: {
        method: "GET",
    ...
    {
      name: "PARTNER PROPERTY | GET Single Branch | Existing or saved id",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
    ...
    {
      name: "PARTNER PROPERTY | GET Single Branch | Invalid UUID",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
    ...
    {
      name: "PARTNER PROPERTY | GET Single Branch | Malformed UUID",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
    ...
    {
      name: "PARTNER PROPERTY | GET Partners | Invalid pagination",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/admin/partners?page=-1&limit=-5"
    ...
    {
      name: "PARTNER PROPERTY | GET Partners | SQL injection search",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/admin/partners?search=' OR '1'='1"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
    ...
    {
      name: "PARTNER PROPERTY | GET Partners | XSS search",
      request: {
        method: "GET",
        header: [{ key: "Authorization", value: "Bearer {{token}}" }],
        url: "{{base_url}}/api/v1/admin/partners?search=<script>alert(1)</script>"
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
    ...
    {
      name: "PARTNER PROPERTY | POST Create Partner | Empty payload validation",
      request: {
        method: "POST",
        header: [
        ...
        url: "{{base_url}}/api/v1/admin/partners",
        body: { mode: "raw", raw: "{}" }
      },
      event: [{ listen: "test", script: { type: "text/javascript", exec: [
        "var json = pm.response.json();",
...
fs.writeFileSync(
  "postman/Novas_PartnerProperty_Regression.postman_collection.json",
  JSON.stringify(collection, null, 2)
);

console.log("Partner Property regression collection created successfully");
